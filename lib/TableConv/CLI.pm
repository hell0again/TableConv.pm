...
sub cmd_reverse {
    ...
    $self->parse_options(
        ...
        "t|type=s" => \my $file_type,
    );
    $self->{source} = shift @args;
    $self->{file_format} = $file_format || "win";
    $self->{file_type} = $file_type || "csv";
    $self->reverse();
}
sub _is_single_worksheet {
    my ($self, $excel) = @_;
    map {
        my $sheet = $excel->{Worksheet}[$_];
...
sub reverse {
    ...
    if (! $self->_is_single_worksheet($excel)) {
        die sprintf("only single sheet accepted. found %d sheet.", scalar @{$excel->{Worksheet}});
    }
    ...
    if ($self->{file_type} =~ /^json$/i) {
        my $json = JSON->new->canonical->pretty;
        print $json->encode($matrix);
    } else {
